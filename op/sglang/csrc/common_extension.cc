...
//   m.impl("gemma_fused_add_rmsnorm", torch::kCUDA, &gemma_fused_add_rmsnorm);
  m.def("rms_norm_dynamic_per_token_quant_custom(Tensor! out, Tensor! out_bf16, Tensor input, Tensor weight, "
...
//   m.impl("es_fp8_blockwise_scaled_grouped_mm", &es_fp8_blockwise_scaled_grouped_mm);
  m.def(
        "fused_mla_RMS_rotary_emb("
        "   Tensor! q, Tensor! latent_cache,  "
        "   Tensor! cos_sin_cache, Tensor! positions,  "
        "   Tensor! norm_weight, Tensor! kv_a,  "
        "   int q_len, int num_local_heads,  "
        "   int kv_lora_rank, int qk_rope_head_dim, int qk_nope_head_dim) ->int  ");		
  m.impl("fused_mla_RMS_rotary_emb", torch::kCUDA, &fused_mla_RMS_rotary_emb);
  

  m.def(
        "fused_mla_normal_kv_element_wise("
        "   Tensor! kv, Tensor! latent_cache,  "
        "   Tensor! k, Tensor! v,  "
        "   int q_len, int num_local_heads,  "
        "   int kv_lora_rank, int qk_nope_head_dim, "
        "   int qk_rope_head_dim, int v_head_dim) -> int");
  m.impl("fused_mla_normal_kv_element_wise", torch::kCUDA, &fused_mla_normal_kv_element_wise);
