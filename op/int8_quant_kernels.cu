...
template<typename T, typename T1, typename VT, typename VT1, int NUM_VT, int type, bool has_swiglu_limit, bool has_weight> 
__global__ void silu_and_mul_mask_quant_pack(T* input, T* output,T1* mask, int mask_size, int64_t grid_size, int64_t num_tokens, int64_t hidden_size, int64_t out_stirde, int blockDim_x, float swiglu_limit = 0.0, T* weight = nullptr)
{
...
void launch_silu_mul_quant_pack(T* input, T* output, T1* mask, int64_t num_tokens, int64_t hidden_size, int64_t out_stride, int64_t mask_size,cudaStream_t stream, float swiglu_limit = 0.0, T* weight = nullptr) {
    ...
    if(mask_size == 1 && N == 8&&(inner_hidden_size & (N - 1)) == 0 && (out_stride & (N -1)) == 0) {
        ...
        if(inner_hidden_size <= 64 * N) {
            ...
            if(weight != nullptr) {
              if(swiglu_limit > 0.0) {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 1, type, true, true><<<gridsize, NUM_THREADS,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, NUM_THREADS, swiglu_limit, weight);
              } else {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 1, type, false, true><<<gridsize, NUM_THREADS,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, NUM_THREADS, swiglu_limit, weight);
              }
            } else {
              if(swiglu_limit > 0.0) {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 1, type, true, false><<<gridsize, NUM_THREADS,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, NUM_THREADS, swiglu_limit, weight);
              } else {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 1, type, false, false><<<gridsize, NUM_THREADS,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, NUM_THREADS, swiglu_limit, weight);
              }
            }
        } else if(inner_hidden_size <= 128 * N) {
            ...
            if(weight != nullptr) {
              if(swiglu_limit > 0.0) {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 1, type, true, true><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              } else {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 1, type, false, true><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              }
            } else {
              if(swiglu_limit > 0.0) {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 1, type, true, false><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              } else {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 1, type, false, false><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              }
            }
        } else if(inner_hidden_size <= base*2) {
            ...
            if(weight != nullptr) {
              if(swiglu_limit > 0.0) {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 2, type, true, true><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              } else {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 2, type, false, true><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              }
            } else {
              if(swiglu_limit > 0.0) {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 2, type, true, false><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              } else {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 2, type, false, false><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              }
            }
        } else if(inner_hidden_size <= base * 3) {
            ...
            if(weight != nullptr) {
              if(swiglu_limit > 0.0) {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 3, type, true, true><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              } else {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 3, type, false, true><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              }
            } else {
              if(swiglu_limit > 0.0) {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 3, type, true, false><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              } else {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 3, type, false, false><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              }
            }
        } else if(inner_hidden_size <= base * 4) {
            ...
            if(weight != nullptr) {
              if(swiglu_limit > 0.0) {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 4, type, true, true><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              } else {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 4, type, false, true><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              }
            } else {
              if(swiglu_limit > 0.0) {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 4, type, true, false><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              } else {
                silu_and_mul_mask_quant_pack<T, T1, float4, float2, 4, type, false, false><<<gridsize, blocksize,0,stream>>>(input, output, mask, mask_size, gridsize, num_tokens, inner_hidden_size, out_stride, blocksize, swiglu_limit, weight);
              }
            }
        } else {
            assert(0);
...
__global__ void silu_and_mul_reorder_quant(const T* input, const float* input_scale, int8_t* output, float* scale, const T_IDS* reorder_topk_ids, int start_expert_id, int end_expert_id, int64_t hidden_size)
{
    ...
    int const tid = threadIdx.x;
    int stride = NUM_THREADS * N;
    T_IDS reorder_tokens = reorder_topk_ids[blockIdx.x];
    if(reorder_tokens < start_expert_id || reorder_tokens > end_expert_id) return;
    float value_scale = 1;
    if constexpr(HAS_SCALE) {
      value_scale = input_scale[reorder_tokens - start_expert_id];
...
template<typename T, typename TYPE_IDS, bool HAS_SCALE, int NUM_THREADS> 
__global__ void silu_and_mul_common_reorder(const T* input, const float* input_scale, T* output, const TYPE_IDS* reorder_topk_ids, int start_expert_id, int end_expert_id, int64_t hidden_size)
{
    int const tid = threadIdx.x;
    TYPE_IDS reorder_tokens = reorder_topk_ids[blockIdx.x];
    if(reorder_tokens < start_expert_id || reorder_tokens > end_expert_id) return;
    ...
    const T* ptr_input0 = input + token_idx * hidden_size2;
    const T* ptr_input1 = ptr_input0 + hidden_size;
    T* ptr_output = output + token_idx * hidden_size;
    float value_scale = 1;
    if constexpr(HAS_SCALE) {
      value_scale = input_scale[reorder_tokens - start_expert_id];
      value_scale = 1.0 / value_scale;
    ...
    for(int i = tid; i < hidden_size; i += NUM_THREADS) {
      ...
      ptr_output[i] = float_to_dstT<T>(gate_up);
    }
}

template<typename T, typename TYPE_IDS, bool HAS_SCALE>
void launch_silu_mul_reorder_topk(const T* input, const float* input_scale, T* output, const TYPE_IDS* reorder_topk_ids, int start_expert_id, int end_expert_id, int64_t num_tokens, int64_t hidden_size,cudaStream_t stream) {
  int64_t inner_hidden_size = hidden_size / 2;
  int blocksize = 512;
...
void static_scaled_int8_quant(at::Tensor& out,          // [..., hidden_size]
                              ...
                              c10::optional<at::Tensor> const& azp) {
...
void dynamic_scaled_int8_quant(
    ...
    at::Tensor const& input,  // [..., hidden_size]
    at::Tensor& scales, c10::optional<at::Tensor> const& azp) {
  TORCH_CHECK(input.is_contiguous());
  TORCH_CHECK(out.is_contiguous());
      ...
      input.scalar_type(), "dynamic_scaled_int8_quant_kernel", [&] {
        if (!azp) {
          ...
          if(hidden_size <= 4096) {
            ...
            int blocksize = 512;
            dynamic_scaled_int8_quant_kernel_reg_opt<scalar_t, float, float4, float4, false><<<grid, blocksize, 0, stream>>>(input.data_ptr<scalar_t>(),out.data_ptr<int8_t>(),scales.data_ptr<float>(),hidden_size,blocksize, num_tokens);
            ...
            dynamic_scaled_int8_quant_kernel_lh_opt<scalar_t, float, float4, float2, 3,1024, false><<<grid, 1024, 0, stream>>>(input.data_ptr<scalar_t>(),out.data_ptr<int8_t>(),scales.data_ptr<float>(),hidden_size, num_tokens);
          } else if (hidden_size > 8064 && ((hidden_size & (n - 1)) == 0 && n == 8)) {
            int blocksize = 1024;
          ...
          dynamic_scaled_int8_azp_quant_kernel<scalar_t, float, int32_t, false>
              <<<grid, block, 0, stream>>>(
                  input.data_ptr<scalar_t>(), out.data_ptr<int8_t>(),
  ...
  MOE_DISPATCH_FLOATING_TYPES(
      input.scalar_type(), "dynamic_scaled_int8_quant_kernel_mask", [&] {
        if (!azp) {
          ...
          if(mask_size < 16) {
            ...
            if(hidden_size <= 4096 && ((hidden_size & (n - 1)) == 0) && n == 8) {
              ...
              dynamic_scaled_int8_quant_mask_kernel_sreg_opt<scalar_t, float, float4, float2><<<gridsize, blocksize, 0, stream>>>(input.data_ptr<scalar_t>(),out.data_ptr<int8_t>(),scales.data_ptr<float>(),hidden_size,blocksize,num_tokens_batch,mask_size,gridsize,mask.data_ptr<int>());
                ...
                int blocksize = 512;
              dynamic_scaled_int8_quant_mask_kernel_reg_opt<scalar_t, float, float4, float4><<<gridsize, blocksize, 0, stream>>>(input.data_ptr<scalar_t>(),out.data_ptr<int8_t>(),scales.data_ptr<float>(),hidden_size,blocksize,num_tokens_batch,mask_size, gridsize,mask.data_ptr<int>());
            } else if(hidden_size <= 8064 && (hidden_size & (n - 1)) == 0 && n == 8) {
                int blocksize = 512;
                dynamic_scaled_int8_quant_mask_kernel_sm_opt<scalar_t, float, float4, float2><<<gridsize, blocksize, 0, stream>>>(input.data_ptr<scalar_t>(),out.data_ptr<int8_t>(),scales.data_ptr<float>(),hidden_size,blocksize,num_tokens_batch,mask_size, gridsize, mask.data_ptr<int>());
            } else if(hidden_size > 8064 && ((hidden_size & (n - 1)) == 0 && n == 8)){
                int blocksize = 1024;
                dynamic_scaled_int8_quant_mask_kernel_opt<scalar_t, float,float4,float2>
              ...
              dynamic_scaled_int8_quant_mask_kernel<scalar_t, float>
            ...
            if(hidden_size <= 4096 && ((hidden_size & (n - 1)) == 0) && n == 8) {
              if(hidden_size > 256*n) {
                dynamic_scaled_int8_quant_kernel_sreg_opt<scalar_t, float, float4, float2,512,true><<<grid, 512, 0, stream>>>(input.data_ptr<scalar_t>(),out.data_ptr<int8_t>(),scales.data_ptr<float>(),hidden_size, num_tokens_batch, mask.data_ptr<int>());
              } else if(hidden_size > 128*n) {
                dynamic_scaled_int8_quant_kernel_sreg_opt<scalar_t, float, float4, float2,256,true><<<grid, 256, 0, stream>>>(input.data_ptr<scalar_t>(),out.data_ptr<int8_t>(),scales.data_ptr<float>(),hidden_size, num_tokens_batch, mask.data_ptr<int>());
              } else if(hidden_size > 64*n) {
                dynamic_scaled_int8_quant_kernel_sreg_opt<scalar_t, float, float4, float2,128,true><<<grid, 128, 0, stream>>>(input.data_ptr<scalar_t>(),out.data_ptr<int8_t>(),scales.data_ptr<float>(),hidden_size, num_tokens_batch, mask.data_ptr<int>());
              } else {
                dynamic_scaled_int8_quant_kernel_sreg_opt<scalar_t, float, float4, float2,64,true><<<grid, 64, 0, stream>>>(input.data_ptr<scalar_t>(),out.data_ptr<int8_t>(),scales.data_ptr<float>(),hidden_size, num_tokens_batch, mask.data_ptr<int>());
              }
            } else if(hidden_size > 4096 &&hidden_size <= 8192 && ((hidden_size & (2*n - 1)) == 0) && n == 8) {
              int blocksize = 512;
              dynamic_scaled_int8_quant_kernel_reg_opt<scalar_t, float, float4, float4, true><<<grid, blocksize, 0, stream>>>(input.data_ptr<scalar_t>(),out.data_ptr<int8_t>(),scales.data_ptr<float>(),hidden_size,blocksize,num_tokens_batch,mask.data_ptr<int>());
            } else if(hidden_size <= 8064 && (hidden_size & (n - 1)) == 0 && n == 8) {
              int blocksize = 512;
              dynamic_scaled_int8_quant_kernel_sm_opt<scalar_t, float, float4, float2, true><<<grid, blocksize, 0, stream>>>(
              ...
              int blocksize = 1024;
              dynamic_scaled_int8_quant_kernel_opt<scalar_t, float,float4,float2, true>
                      <<<grid, blocksize, 0, stream>>>(
                          input.data_ptr<scalar_t>(),out.data_ptr<int8_t>(),scales.data_ptr<float>(),hidden_size,blocksize, num_tokens_batch, mask.data_ptr<int>());
            } else {
                dynamic_scaled_int8_quant_kernel<scalar_t, float,true>
                    <<<grid, block, 0, stream>>>(
                        input.data_ptr<scalar_t>(), out.data_ptr<int8_t>(),
          ...
          dynamic_scaled_int8_azp_quant_kernel<scalar_t, float, int32_t,true>
              <<<grid, block, 0, stream>>>(
                  input.data_ptr<scalar_t>(), out.data_ptr<int8_t>(),
                  scales.data_ptr<float>(), azp->data_ptr<int32_t>(),
...
{
  DEBUG_TRACE_PARAMS(out, input, &mask, _swiglu_limit, weight);
  DEBUG_DUMP_PARAMS(out, input, &mask, _swiglu_limit, weight);
  TORCH_CHECK(input.is_contiguous());
  TORCH_CHECK(out.is_contiguous());
  TORCH_CHECK(mask.is_contiguous());
...
void fused_silu_mul_reordered_topk_interface(
    at::Tensor& out,
    at::Tensor const& input,
    at::Tensor const& reorder_topk_ids,
    at::Tensor const& w2_scale,
    ...
    int64_t end_expert_id)
{
  ...
  if(w2_scale.numel() == 0) {
    switch(reorder_topk_ids.element_size()) {
      case 8:
        MOE_DISPATCH_FLOATING_TYPES(input.scalar_type(), "launch_silu_mul_reorder_topk", [&] {
          launch_silu_mul_reorder_topk<scalar_t, int64_t, false>(input.data_ptr<scalar_t>(), nullptr, out.data_ptr<scalar_t>(), reorder_topk_ids.data_ptr<int64_t>(), start_expert_id, end_expert_id, num_tokens, hidden_size, stream);
        });
      break;
      case 4:
        MOE_DISPATCH_FLOATING_TYPES(input.scalar_type(), "launch_silu_mul_reorder_topk", [&] {
          launch_silu_mul_reorder_topk<scalar_t, int, false>(input.data_ptr<scalar_t>(), nullptr, out.data_ptr<scalar_t>(), reorder_topk_ids.data_ptr<int>(), start_expert_id, end_expert_id, num_tokens, hidden_size, stream);
        });
      break;
      default:
      return;
    ...
    switch(reorder_topk_ids.element_size()) {
      case 8:
        MOE_DISPATCH_FLOATING_TYPES(input.scalar_type(), "launch_silu_mul_reorder_topk", [&] {
          launch_silu_mul_reorder_topk<scalar_t, int64_t, false>(input.data_ptr<scalar_t>(), w2_scale.data_ptr<float>(), out.data_ptr<scalar_t>(), reorder_topk_ids.data_ptr<int64_t>(), start_expert_id, end_expert_id, num_tokens, hidden_size, stream);
        });
      break;
      case 4:
        MOE_DISPATCH_FLOATING_TYPES(input.scalar_type(), "launch_silu_mul_reorder_topk", [&] {
          launch_silu_mul_reorder_topk<scalar_t, int, false>(input.data_ptr<scalar_t>(), w2_scale.data_ptr<float>(), out.data_ptr<scalar_t>(), reorder_topk_ids.data_ptr<int>(), start_expert_id, end_expert_id, num_tokens, hidden_size, stream);
        });
      break;
      default:
      return;
