...
template <typename scalar_t, bool IS_NEOX>
inline __device__ void batch_apply_token_rotary_embedding(
...
template <typename scalar_t, bool IS_NEOX>
inline __device__ void batch_apply_rotary_embedding(
    scalar_t* __restrict__ query,  // [batch_size, seq_len, num_heads,
                                   // head_size] or [num_tokens, num_heads,
                                   // head_size]
    scalar_t* __restrict__ key,    // nullptr or
  ...
  for (int i = threadIdx.x; i < nq; i += blockDim.x) {
    ...
    const int rot_offset = i % embed_dim;
    batch_apply_token_rotary_embedding<scalar_t, IS_NEOX>(
...
__global__ void batched_rotary_embedding_kernel(
  ...
  batch_apply_rotary_embedding<scalar_t, IS_NEOX>(
      query, key, cache_ptr, head_size, num_heads, num_kv_heads, rot_dim,
