...
# from mcoplib.op import fused_silu_mul_dq_mask_fp8_quant
import triton
...
def silu_and_mul_masked_fwd(
    ...
    for i  in range(float32_ref.shape[0]):
        for j in range(mask_value):
            ref[i][j][0] = float32_ref[i][j][(hidden_size)//4]*127
            dst[i][j][0] = float32_dst[i][j][(hidden_size)//4]*448
    # print(f'{ref=}')
    # print(f'{dst=}')
    assert(torch.allclose(ref, dst, rtol=1e-02, atol=1e-01, equal_nan=True))
    print("done")
