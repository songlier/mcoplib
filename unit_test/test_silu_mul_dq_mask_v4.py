...
def _silu_and_mul_masked_kernel(
    ...
    weight_offset = weights_ptr + offs_in_d
    ...
    for token_index in tl.range(
        ...
        w = tl.load(weight_offset, mask = offs_in_d < size_n, other=0.0).to(tl.float32)
        gate_up = gate_up * w
        # Store BF16 result
        tl.store(
...
def silu_and_mul_masked_fwd(
    ...
    swiglu_limit = 2.0
    # weights=torch.randn(size_n, device='cuda', dtype=torch.bfloat16)
    num_warps = 4
    NUM_STAGES = 3
    ...
    fused_silu_mul_dq_mask_quant(dst, input, masked_m, swiglu_limit, weights)
...
def test_silu_and_mul_masked_fwd(m, n, dtype,warmup=1, rep=1):
    ...
    for mask_id in [64,128,256,512]:
            ...
            weights = torch.randn(n//2, device='cuda', dtype=torch.bfloat16)
            masked_m = torch.full((num_groups,), mask_id, dtype=torch.int32, device='cuda')
            # print(f"mask_m:{masked_m}")
            silu_and_mul_masked_fwd(x, x_out, masked_m,2.0, weights)
...
if __name__ == "__main__":
    ...
    print("使用设备:", device)
    print("v4")
    test_silu_and_mul_masked_fwd(3072,3072,torch.bfloat16)
