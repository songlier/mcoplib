...
macro(clear_torch_arches CUDA_ARCH_FLAGS)

    string(REGEX REPLACE "-mllvm -structurizecfg-skip-uniform-regions=true" "" CMAKE_CUDA_FLAGS ${CMAKE_CUDA_FLAGS})
    # Remove all `-gencode` flags from `CMAKE_CUDA_FLAGS` since they will be modified
    # and passed back via the `CUDA_ARCHITECTURES` property.
    string(REGEX REPLACE "-gencode arch=[^ ]+ *" "" CMAKE_CUDA_FLAGS
      ${CMAKE_CUDA_FLAGS})
    string(REGEX REPLACE "-mllvm -enable-misched[^ ]*" "" CMAKE_CUDA_FLAGS
      ${CMAKE_CUDA_FLAGS})
    string(REGEX REPLACE "-02" "" CMAKE_CUDA_FLAGS
      ${CMAKE_CUDA_FLAGS})
    string(REGEX REPLACE "-g" "" CMAKE_CUDA_FLAGS
      ${CMAKE_CUDA_FLAGS})
    string(REGEX REPLACE "-D__CUDACC_EXTENDED_LAMBDA__" "" CMAKE_CUDA_FLAGS
      ${CMAKE_CUDA_FLAGS})
    string(REGEX REPLACE "-DONNX_NAMESPACE[^ ]*" "" CMAKE_CUDA_FLAGS
      ${CMAKE_CUDA_FLAGS})
    string(REGEX REPLACE "-mllvm -metaxgpu-disable-bsm-offset[^ ]*" "" CMAKE_CUDA_FLAGS
      ${CMAKE_CUDA_FLAGS})
    string(REGEX REPLACE "-mllvm -metaxgpu-inlinescope[^ ]*" "" CMAKE_CUDA_FLAGS
      ${CMAKE_CUDA_FLAGS})
    string(REGEX REPLACE "-mllvm -metaxgpu-hoist-arrive[^ ]*" "" CMAKE_CUDA_FLAGS
      ${CMAKE_CUDA_FLAGS})
    string(REGEX REPLACE "-mllvm -structurizecfg-skip-uniform-regions[^ ]*" "" CMAKE_CUDA_FLAGS
      ${CMAKE_CUDA_FLAGS})
    string(REGEX REPLACE "-mllvm -metaxgpu-GridDim-UseLdu[^ ]*" "" CMAKE_CUDA_FLAGS
      ${CMAKE_CUDA_FLAGS})
endmacro()
...
function (define_gpu_extension_target GPU_MOD_NAME)
  ...
  target_link_libraries(${GPU_MOD_NAME} PRIVATE torch c10 torch_cpu torch_python torch_cuda)
