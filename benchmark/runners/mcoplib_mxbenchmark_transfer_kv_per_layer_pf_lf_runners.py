...
class Transfer_kv_per_layer_pf_lf_runner(OpBenchmarkBase):
    def __init__(self, name, config):
        super().__init__(name, config)
        self.num_tokens = config.get("num_tokens", 1024)
        self.num_pages = config.get("num_pages", 4096)
        self.item_elements = config.get("item_elements", 128)
        ...
        self.layer_id = 0  # Defaulting to 0 for benchmark

    def define_metrics(self, state):
        state.add_summary("Op", self.name)
        state.add_summary("dtype", self.config.get("dtype", str(self.dtype)))
        state.add_summary("Shape", f"(Tokens:{self.num_tokens} Pages:{self.num_pages} Dim:{self.item_elements})")
        
        # Elements processed: tokens * items_per_token * 2 (for K and V)
    ...
    def prepare_and_get_launcher(self, dev_id, tc_s):
        ...
        with torch.cuda.stream(tc_s):
            src_k = torch.randn((self.num_pages, self.num_layers, self.item_elements), dtype=self.dtype, device=dev)
            src_v = torch.randn((self.num_pages, self.num_layers, self.item_elements), dtype=self.dtype, device=dev)
            
            # dst_layout_dim is treated as 0 in C++, so it's a 1D sequence mapping to the specific layer base
            ...
            dst_indices = torch.randint(0, self.num_pages, (self.num_tokens,), dtype=torch.long, device=dev)
    ...
    def run_verification(self, dev_id):
        ...
        # Source indices can have duplicates (concurrent reads are fine)
        src_indices = torch.randint(0, self.num_pages, (self.num_tokens,), dtype=torch.long, device=dev)
        # Destination indices MUST be unique to prevent write race conditions
        ...
        for i in range(self.num_tokens):
            ...
            s_offset = (s_idx * page_dim_elements) + (self.layer_id * self.item_elements)
            d_offset = d_idx * self.item_elements
            dst_k_ref.view(-1)[d_offset : d_offset + self.item_elements] = src_k.view(-1)[s_offset : s_offset + self.item_elements]
            dst_v_ref.view(-1)[d_offset : d_offset + self.item_elements] = src_v.view(-1)[s_offset : s_offset + self.item_elements]
