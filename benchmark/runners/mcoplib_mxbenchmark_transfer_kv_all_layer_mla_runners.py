...
class Transfer_kv_all_layer_mla_runner(OpBenchmarkBase):
    ...
    def _create_tensors(self, dev):
        # 优化 1：Bulk Allocation (大块分配)。直接申请一整块连续内存，极大降低 Allocator 压力
        src_bulk = torch.randn((self.num_layers, self.num_tokens, self.elements_per_item), dtype=self.dtype, device=dev)
        dst_bulk = torch.zeros_like(src_bulk)
        
        # 使用视图切片代替独立生成张量 (它们共享 src_bulk/dst_bulk 的底层显存)
        src_layers = [src_bulk[i] for i in range(self.num_layers)]
        dst_layers = [dst_bulk[i] for i in range(self.num_layers)]
        
        src_ptrs = self._to_uint64_ptr(src_layers, dev)
        dst_ptrs = self._to_uint64_ptr(dst_layers, dev)
        ...
        # 返回 bulk 张量以防被 Python GC 回收导致指针悬空
        return src_bulk, dst_bulk, src_layers, dst_layers, src_ptrs, dst_ptrs, src_indices, dst_indices

    def prepare_and_get_launcher(self, dev_id, tc_s):
        dev = f'cuda:{dev_id}'
        self.tensors = self._create_tensors(dev)
        # 解包 (忽略前两个 bulk 引用)
        _, _, _, _, src_ptrs, dst_ptrs, src_indices, dst_indices = self.tensors
        
        return self.make_launcher(
            dev_id,
            torch.ops.sgl_kernel.transfer_kv_all_layer_mla,
            src_ptrs, dst_ptrs, src_indices, dst_indices,
            self.item_byte_size, self.num_layers,
            self.block_quota, self.num_warps_per_block
        )

    def run_verification(self, dev_id):
        dev = f'cuda:{dev_id}'
        src_bulk, dst_bulk, src_layers, dst_layers, src_ptrs, dst_ptrs, src_indices, dst_indices = self._create_tensors(dev)
        ...
        for i in range(self.num_layers):
            pass_k, diff_k = self.check_diff(dst_layers[i], src_layers[i])
            is_passed = is_passed and pass_k
            max_diff = max(max_diff, diff_k)
            
        # 优化 2：强制垃圾回收和清空显存缓存，防止 10 次 Warmup 把显存撑爆
        del src_bulk, dst_bulk, src_layers, dst_layers, src_ptrs, dst_ptrs, src_indices, dst_indices
