...
namespace deepseek_moe_gate {
    ...
    __device__ __forceinline__ void warpSortDescending(scalar_t (&idx_and_weight)[2], int tid) {
        ...
        for (int width = 2; width <= WARP_SIZE; width <<=1) {
            for (int step = width >> 1; step > 0; step >>=1) {
                ...
                if constexpr (std::is_same_v<scalar_t, half> || std::is_same_v<scalar_t, at::Half>)
                {
                    weight_gt = __hgt(other_weight_bits, current_weight_bits);
                    weight_eq = __heq(other_weight_bits, current_weight_bits);
                ...
                if constexpr (std::is_same_v<scalar_t, half> || std::is_same_v<scalar_t, at::Half>) {
                    swap = (cond & (weight_gt | (weight_eq & index_lt))) |
                            (!cond & (__hlt(other_weight_bits, current_weight_bits) | (weight_eq & (other_index > current_index))));
    ...
    template<class scalar1_t, typename scalar2_t, int NUM_GROUPS = 8, int TOPK=8>
    __device__ __forceinline__ int moe_topk_block_phase1(const scalar1_t* w, const scalar2_t* bias, scalar1_t* global_score, int tid) {
        using scalar_t = typename ChooseLargerType<scalar1_t, scalar2_t>::Type; // scalar_t only used for sort.
        scalar_t idx_and_weight[2]; //Stores weight and index for further top1
        scalar_t idx_and_weight_2[2]; //used for top8 calculation

        float fw = scalar2float<scalar1_t>(w[tid]);
        scalar1_t fw_sim = float2scalar<scalar1_t>(__builtin_rcpf((1.0f + __expf(-fw))));
        scalar_t fw_bf16 =  fw_sim + float2scalar<scalar_t>(scalar2float<scalar2_t>(bias[tid]));
    ...
    template<class scalar1_t, typename scalar2_t, int NUM_EXPERTS=384, int NUM_GROUPS = 1, int TOPK=8>
    __device__ __forceinline__ int moe_topk_1group_block_phase1(const scalar1_t* w, const scalar2_t* bias, scalar1_t* global_score, int tid) {
    ...
    template<class scalar_t, typename scalar2_t, int TOPK=8>
    __device__ __forceinline__ void deepseek_topk_phase2(int top_k_idx, bool renormalize, scalar_t *global_score, scalar2_t* topk_indices, float* topk_w, float scale_factor = 1.0f)
    {
        int tid = threadIdx.x;
        if (tid < TOPK) {
    ...
    template<class scalar_t, typename scalar2_t, int NUM_SHARED_EXPERTS=0, int NUM_EXPERTS=256, int TOPK=8>
    __device__ __forceinline__ void sglang_topk_phase2(int top_k_idx, bool renormalize, scalar_t *global_score, scalar2_t* topk_indices, float* topk_w, float scale_factor = 1.0f)
    {
        int tid = threadIdx.x;
        if(tid >= 32) return;
        int last_pos = TOPK;
        ...
        if(tid == (TOPK - 1)){
            ...
            {
                int left_zeros = __clz(NUM_SHARED_EXPERTS);
                uint32_t mask = (0x1 << (32-left_zeros)) - 1;
                uint32_t seed = (clock64() & 0xffffffff); // + blockIdx.x; each block get different clock64
                seed = (seed * 0x5DEECE66D + 0xB) & mask; // erand48
                if (seed >= NUM_SHARED_EXPERTS)
                    seed = seed - NUM_SHARED_EXPERTS;
                top_k_idx = NUM_EXPERTS + seed;
            }
        }
        if(tid < TOPK)
            topk_indices[tid] = top_k_idx;
    ...
    template<typename scalar1_t, typename scalar2_t, typename scalar3_t, int NUM_SHARED_EXPERTS=0, int NUM_EXPERTS=256, int NUM_GROUPS=8, int TOPK=8, int moe_type = 0>
    __global__ void fused_moegate_topk(const scalar1_t* w, const scalar2_t* bias, float* topk_w, scalar3_t* topk_indices, bool renormalize, float scale_factor = 1.0f) {
        int tid = threadIdx.x;
        int bdx = blockIdx.x;
        if constexpr (std::is_same_v<scalar1_t, double>)
        {
            if((tid == 0)&&(bdx == 0))
                printf("%s unsupported double type.\n", __func__);
            return;
        }
        __shared__ scalar1_t global_score[NUM_EXPERTS];
        int top_k_idx = 0;
        if constexpr (NUM_GROUPS == 1) {
            top_k_idx = moe_topk_1group_block_phase1<scalar1_t, scalar2_t, NUM_EXPERTS, 1, TOPK>(w + bdx * NUM_EXPERTS, bias, global_score, tid);
        } else {
            top_k_idx = moe_topk_block_phase1<scalar1_t, scalar2_t>(w + bdx * NUM_EXPERTS, bias, global_score, tid);
        }
        if constexpr (moe_type == 0)
            deepseek_topk_phase2<scalar1_t, scalar3_t>(top_k_idx, renormalize, global_score, topk_indices + bdx * TOPK, topk_w + bdx * TOPK, scale_factor);
        else if constexpr (moe_type == 1)
            sglang_topk_phase2<scalar1_t, scalar3_t, NUM_SHARED_EXPERTS, NUM_EXPERTS, TOPK>(top_k_idx, renormalize, global_score, topk_indices + bdx * TOPK, topk_w + bdx * TOPK, scale_factor);
        else
            if((tid == 0) && (bdx == 0))
                printf("%s unsupported MOEGATE_TYPE %d\n", moe_type);
    }
